...
description: >-
  todo-list-with-ai 프로젝트 구조를 분석해 현재 없는 신기능 3개를 제안하고,
...
| 대상 | 경로 |
|------|------|
| 페이지·레이아웃 | `src/app/` (page.tsx, layout.tsx) |
| 컴포넌트 | `src/app/components/` 전체 |
| 타입·훅 | `src/app/components/types.ts`, `src/app/components/useLocalStorage.ts` |
| 글로벌 스타일 | `src/app/globals.css` |
| 설정 | `next.config.ts`, `tsconfig.json` |

현재 기능을 간결히 정리한다 (예: 할 일 추가/삭제, 필터링, localStorage 동기화 등).
...
3. 타입 일관성 유지 (`src/app/components/types.ts` 기준)

## Step 4 — 빌드 검증

```bash
cd todo-list-with-ai && npm run build
```
